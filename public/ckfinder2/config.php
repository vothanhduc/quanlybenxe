...
$config['authentication'] = function () {
    if (isset($_COOKIE['id']) || isset($_SESSION['id'])) {
        if (isset($_COOKIE['id'])) {
            $tbl_user = unserialize($_COOKIE['tbl_user']);
            $quyensd = $tbl_user['quyensd'];
        } else if (isset($_SESSION['id']))
            $quyensd = $_SESSION['quyensd'];
        if (in_array($quyensd, array(1, 2)))
            return true;
        else
            return false;
    } else
...
$config['licenseKey'] = "";
if (file_exists("/../../ck3key.json")) {
    $fp = fopen("/../../ck3key.json", "r");
    $keyck3 = json_decode(fread($fp, filesize("/../../ck3key.json")), true);
    if (isset($keyck3[$_SERVER['HTTP_HOST']]))
        $config['licenseKey'] = $keyck3[$_SERVER['HTTP_HOST']];
}
...
$config['pluginsDirectory'] = '/plugins';
